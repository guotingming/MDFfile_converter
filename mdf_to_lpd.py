...
def calculate_Pout(a2_real,a2_imag,b2_real,b2_imag):
    ...
    return Pout
def calculate_PAE(Pout,V2,I2):
    Pout=float(Pout)
    V2=float(V2)
    I2=float(I2)
    PAE=(math.pow(10,Pout/10)/1000)/(V2*I2)*1000*100
    return PAE
def converter_data(filename,savename_dir):
    ...
    print(len(DATA))
    try:
        DATA=DATA.reshape(int(len(DATA)/13),13)
    except Exception as e:
        print(e)
        print('数据不完整')
        error='数据不完整'
    ...
    #print(DATA)
    try:
        DATA=pd.DataFrame(DATA,columns=['realload','imagload','f0_drive','V1','I1','V2','I2','a1_real','a1_imag','a2_real','a2_imag'
                                        ,'b2_real','b2_imag'])
    except Exception as e:
        print(e)
    ...
    try:
        ...
        #计算Pout
        a2_b2_data=DATA.iloc[:,9:13].values
        #print(a2_b2_data)
        Pout_data=[]
        for i in a2_b2_data:
